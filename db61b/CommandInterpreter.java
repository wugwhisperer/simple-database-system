...
class CommandInterpreter {
    ...
    void insertStatement() {
        ...
        while (true) {
            ...
            while (!_input.nextIf(")")) {
                next = _input.next().replaceAll("\'", "");
                columnstring.append(next);
            }
            String colstring = columnstring.toString();
            values = colstring.split(",");
            if (values.length != cols) {
                throw error("Invalid number of values: %s", values);
            }
            table.add(values);
            if (_input.nextIf(";")) {
                break;
    ...
    void loadStatement() {
        _input.next("load");
        _name = name();
        Table table = Table.readTable(_name);
        _database.put(_name, table);
        System.out.println(String.format("Loaded %s.db", _name));
        _input.next(";");
    }
    ...
    void printStatement() {
        _input.next("print");
        Table table = tableName();
        System.out.println(String.format("Contents of %s:", _name));
        table.print();
        _input.next(";");
    }
    ...
    Table tableDefinition() {
        ...
        if (_input.nextIf("(")) {
            StringBuilder columnstring = new StringBuilder();
            while (!_input.nextIf(")")) {
                columnstring.append(_input.next());
            }
            String colstring = columnstring.toString();
            String[] colnames = colstring.split(",");
    ...
    Table selectClause() {
        ...
        String colNames = "";
        String tableNames = "";
        ArrayList<Condition> condL = new ArrayList<Condition>();
        while (!_input.nextIf("from")) {
            colNames += _input.next();
        }
        List<String> colList = Arrays.asList(colNames.split(","));
        while (true) {
            tableNames += _input.next();
            if ((_input.nextIs("where")) || (_input.nextIs(";"))) {
                String[] tableList = tableNames.split(",");
                List<Table> tables = new ArrayList<Table>();
                for (int i = 0; i < tableList.length; i++) {
                    tables.add(_database.get(tableList[i]));
                }
                Table[] tableL = tables.toArray(new Table[tableList.length]);
                for (int i = 0; i < tableL.length; i++) {
                    if (tableL[i] == null) {
                        throw error("unknown table: %s", tableList[i]);
                    }
                }

                if (_input.nextIf("where")) {
                    condL = conditionClause(tableL);
                    if (tables.size() > 1) {
                        return tableL[0].select(tableL[1], colList, condL);
                    } else {
                        return tableL[0].select(colList, condL);
                    }
                } else {
                    if (tables.size() > 1) {
                        return tableL[0].select(tableL[1], colList, condL);
    ...
    /** Parse a valid relation value from the token stream, and
     * return the String that it designates, which must be loaded. */
    String relation() {
    ...
    Condition condition(Table... tables) {
        try {
            ...
            while ((!_input.nextIs("and")) && (!_input.nextIs(";"))) {
                literal.append(_input.next());
            }
            String literalstr = literal.toString();
            for (Table table : tables) {
                if (table.findColumn(literalstr) != -1) {
                    Column column2 = new Column(literalstr, table);
                    condition = new Condition(column, relation, column2);
                    break;
                }
            }
            if (condition == null) {
                ...
                literalstr = literalstr.replaceAll("\"", "");
                condition = new Condition(column, relation, literalstr);
            }
            return condition;
        } catch (NullPointerException e) {
            throw error("unknown table: %s", _name);
        }
